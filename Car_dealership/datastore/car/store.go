...
func (s Store) CreateCar(ctx context.Context, car *models.Car) (models.Car, error) {
	_, err := s.db.ExecContext(ctx, "INSERT INTO Car ( ID,Engine,Name,Year,Brand,FuelType) VALUES(?,?,?,?,?,?)",
...
func (s Store) UpdateCar(ctx context.Context, id string, car models.Car) (models.Car, error) {
	_, err := s.db.ExecContext(ctx, "UPDATE Car SET name=?,year=?,Brand=?,FuelType=? WHERE ID=?",
